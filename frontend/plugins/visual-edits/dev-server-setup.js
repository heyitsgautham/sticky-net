      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="sticky-net" -c user.email="dev@sticky-net.local" add "${targetFile}"`);
            execSync(`git -c user.name="sticky-net" -c user.email="dev@sticky-net.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
